...
def log_my_plot_regression(y_test, y_pred, y_pred_sigmoid, y_variable, my_sample_threshold=65):
    ...
    sns.lmplot(x='y_test', y='y_pred', data=pd.DataFrame({'y_test':y_test, 'y_pred':y_pred}), fit_reg=True)
    plt.title('Correlation Plot')
    plt.xlabel(y_variable)
    plt.ylabel(f'predicted {y_variable}')
    wandb.log({"correlation_plot": wandb.Image(plt)})
    plt.close()

    # confusion matrix
    y_pred_bin = my_binarize(y_pred, threshold=my_sample_threshold)
    y_test_bin = my_binarize(y_test, threshold=my_sample_threshold)
    ...
    # convert y_pred to sigmoid
    fpr, tpr, _ = roc_curve(y_test_bin, y_pred_sigmoid)
    roc_auc = auc(fpr, tpr)
...
def train_model_regression(model_name, train_dataloader, test_dataloader, input_size, output_size, n_epochs, batch_size, lr, y_variable, save_model_dir, d_model=512, dim_feedforward = 2048, dropout=0.2, num_heads=4, num_layers=2, SEED=42, my_sample_threshold=65):
    # set initialization
    ...
    for epoch in range(n_epochs):
        ...
        for X, y in tqdm(train_dataloader):
            ...
            train_corr += np.corrcoef(outputs.squeeze().detach().cpu().numpy(), y.detach().cpu().numpy())[0, 1]
            y_preds_prob_list.append(torch.sigmoid(outputs).squeeze().detach().cpu().numpy())
            y_preds_list.append(outputs.squeeze().detach().cpu().numpy())
        ...
        # calcualte binarized results:
        y_preds_bin = my_binarize(y_preds_list_flattened, threshold=my_sample_threshold)
        y_train_bin = my_binarize(y_train_list_flattened, threshold=my_sample_threshold)
        train_auc = roc_auc_score(y_train_bin, y_preds_prob_list_flattened)
        train_accuracy = accuracy_score(y_train_bin, y_preds_bin)
        ...
        with torch.no_grad():
            for X, y in test_dataloader:
                ...
                # test correlation
                test_corr += np.corrcoef(outputs.squeeze().detach().cpu().numpy(), y.detach().cpu().numpy())[0, 1]

                y_preds_bin = my_binarize(outputs.squeeze().detach().cpu().numpy(), threshold=my_sample_threshold)
                y_test_bin = my_binarize(y.detach().cpu().numpy(), threshold=my_sample_threshold)
    
        # save for debugging
        test_df['y_pred'] = outputs.squeeze().detach().cpu().numpy()   
        test_df['y_pred_prob'] = torch.sigmoid(outputs).squeeze().detach().cpu().numpy()
        ...
        test_corr /= len(test_dataloader)
        
        # calculate auc and accuracy
        test_auc = roc_auc_score(y_test_bin, torch.sigmoid(outputs).squeeze().detach().cpu().numpy())
        test_accuracy = accuracy_score(y_test_bin, y_preds_bin)
        
        print(f"Epoch {epoch+1}/{n_epochs}:")
        print(f"Train Loss: {train_loss:.4f} | Train Correlation: {train_corr:.4f} |Train AUC: {train_auc:.4f} | Train Accuracy: {train_accuracy:.4f}")
        print(f"Test Loss: {test_loss:.4f} | Test Correlation: {test_corr:.4f} | Test AUC: {test_auc:.4f} | Test Accuracy: {test_accuracy:.4f}")
        
        wandb.log({'train_loss': train_loss})
        wandb.log({'train_corr': train_corr})
    ...
    log_my_plot_regression(y_test=test_df['y'], y_pred=test_df['y_pred'],y_pred_sigmoid=test_df['y_pred_prob'], y_variable=y_variable, my_sample_threshold=my_sample_threshold)
    ...
    y = y.to(device)
    outputs = model(X.float())
    print('sanity check train correlation: ',np.corrcoef(outputs.squeeze().detach().cpu().numpy(), y.detach().cpu().numpy())[0, 1])
    y_preds_bin = my_binarize(outputs.squeeze().detach().cpu().numpy(), threshold=my_sample_threshold)
    y_bin = my_binarize(y.detach().cpu().numpy(), threshold=my_sample_threshold)
    print('sanity check train auc: ',roc_auc_score(y_bin, y_preds_bin))
    print('sanity check train accuracy: ', accuracy_score(y_bin, y_preds_bin))
 
    return test_corr_list, test_auc_list, test_accuracy_list, model
...
def main(
    ...
    if y_variable == 'age':
        my_sample_threshold = 60
    elif y_variable == 'TScore':
        my_sample_threshold = 65
    elif y_variable =='TScore_scaled':
        # scale the Tscore
        augmented_df['TScore_scaled'] = (augmented_df['TScore'] - augmented_df['TScore'].mean()) / augmented_df['TScore'].std()
        my_sample_threshold = (65 - augmented_df['TScore'].mean()) / augmented_df['TScore'].std()
        print(f'scaled Tscore 65 is {my_sample_threshold}')
    else:
        my_sample_threshold = 0.5 
    ...
    project_name = "heal_rs_transformer_regression_v1"
    ...
    test_corr_list, test_auc_list, test_accuracy_list, model = train_model_regression(model_name, train_dataloader, test_dataloader, input_size=input_size, output_size=output_size, n_epochs=n_ep, batch_size=batch_size, lr=lr, y_variable=y_variable, save_model_dir=save_model_dir, dropout=dropout, dim_feedforward = dim_feedforward, d_model=d_model, num_heads=num_heads, num_layers=num_layers, SEED=SEED, my_sample_threshold=my_sample_threshold) 
    ...
    # torch.save(model.state_dict(), f'{results_file_name}.pt')
    cv_test_df = pd.DataFrame({'correlation': test_corr_list, 'auc':test_auc_list, 'accuracy':test_accuracy_list, 'epoch':range(len(test_auc_list))}).to_csv(f'{results_file_name}.csv')

    print(f"best test corr: {np.mean(test_corr_list):.4f}")
...
if __name__ == "__main__":
   ...
   # create a dictionary of parameters
    ...
    'ROI_scheme': ['schaefer400', 'schaefer200'],
    'lr': [0.0001, 0.00001],
    'd_model': [512, 256],
    'num_heads': [8],
    'num_layers': [6],
    ...
    'SEED': [30, 42, 50],
    'y_variable': ['TScore_scaled']
    }
    print("this is a regression task. Check the output size is the number of classes...")
    output_size = 1
